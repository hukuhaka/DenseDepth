...
class Encoder(nn.Module):
    def __init__(self, args):
        ...
        if args.encoder_model == "densenet":
            self.model = timm.create_model("densenet169", pretrained=True)
        elif args.encoder_model == "efficientnet":
            self.model = timm.create_model("tf_efficientnet_b5_ap", pretrained=True)
        elif args.encoder_model == "mobilevit":
            self.model = timm.create_model("mobilevit_s", pretrained=True)
        elif args.encoder_model == "mobilevitv2":
