...
def image_gradients(img, device):
    ...
    dy = torch.cat([dy, torch.zeros(
                [batch_size, channels, 1, width], device=device, dtype=img.dtype),], dim=2,)
    dy = dy.view(img_shape)

    dx = torch.cat([dx, torch.zeros(
        [batch_size, channels, height, 1], device=device, dtype=img.dtype),], dim=3,)
...
def loss_fn(y_true, y_pred, alpha=1.0, gamma=0.1, device="cuda", maxDepth=10.0):

    l_depth = F.l1_loss(y_true, y_pred)

    l_ssim = torch.clamp(
        (1-ssim(y_pred, y_true, maxDepth))*0.5, min=0, max=1)
    ...
    net_loss = (alpha*l_ssim) + (alpha*torch.mean(l_edges)) + (gamma*torch.mean(l_depth))
    return net_loss